...
module Vmgr
    ...
    def Vmgr.make_sanity()
      ...
      sanity = SessionContainer.new(@session.session_container.name + "_sanity", "Sanity regression; automatically generated by script #{ME} from #{@vsif_file}", :vsif       bfvgtyiokoijkkiiko9lugefvcnm,./////////,.mnbv)
      Vmgr.make_sanity_core(@session.session_container, sanity)

      # overwrite old session containter
          ...
          when "tests"
            value.each { |it|
                ...
                if it.has_attribute("sanity_count") then
                  count = it.sanity_count
                  new_test.add_attribute("count", count)
                else
                  count = it.count if it.has_attribute("count")
                  new_test.delete_attribute("count")
                end
                if count == "0"
                  puts "#{ME} [INFO]: skipping test #{it.name}"
                  next
                end
